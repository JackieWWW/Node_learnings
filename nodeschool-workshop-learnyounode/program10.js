...
//   file:///Users/peekay/.nvm/v0.10.40/lib/node_modules/learnyounode/node_apidoc/net.html
...
function getDateNow() {
    var now = new Date();
    return fillZero(now.getFullYear()) + '-'
        +  fillZero(now.getMonth() + 1) + '-'     // starts at 0
        +  fillZero(now.getDate()) + ' '  // returns the day of month
        +  fillZero(now.getHours()) + ':' 
        +  fillZero(now.getMinutes());
}

var server = net.createServer(function (socket) {
    socket.end(getDateNow() + '\n');
});
